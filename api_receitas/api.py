...
from receitas.models import Ingrediente, Receita, Utilizador
...
api = NinjaAPI(
    ...
    description="API para gestao de receitas culinarias, utilizadores e ingredientes.",
    auth=AuthAPIKey(),
)


@api.get("/ingredientes", response=list[IngredienteOutputSchema], tags=["Ingredientes"])
def lista_ingredientes(request, nome: str = None, limit: int = 10, offset: int = 0):
...
@api.get(
    ...
    response=IngredienteOutputSchema,
    tags=["Ingredientes"],
)
...
def lista_utilizadores(request, nome: str = None, limit: int = 10, offset: int = 0):
...
@api.get(
    ...
    response=UtilizadorOutputSchema,
    tags=["Utilizadores"],
)
...
def lista_receitas(request, titulo: str = None, limit: int = 10, offset: int = 0):
